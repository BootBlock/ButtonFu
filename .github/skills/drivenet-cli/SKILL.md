...
By default, that session shows a non-activating warning toast with a 1.5-second progress countdown before automation begins.
...
- Batch same-process work into one `batch` command whenever possible so the attach cost and 1.5-second warning toast happen only once.
- Use `playback` as the CLI replay entry point for recorded batch JSON, YAML suites, and YAML manifests.
- After the first user-visible warning has already been shown, continuation CLI commands can pass `--continuation` (alias: `--no-session-start-warning`) to skip the repeated warning toast for that command.
